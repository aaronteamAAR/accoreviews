    ...
    <>
      ...
      <div>
        <Drawer placement="left" onClose={onClose} isOpen={isOpen}>
          ...
          <DrawerContent 
            bg="white" 
            width={'container.sm'} // Full width on small screens, 50% width on large screens
          >
              <DrawerHeader>
            ...
            <DrawerBody>
              <ul className="space-y-5">
                {links.map(link => (
                  <li key={link.id} className="text-[var(--color-text)] h-10 w-40 px-3 flex items-center">
              ...
              <div className="mx-10 sm:mx-5 s:mx-5 border-b border-[var(--color-secondary-2)]">
                <div className="flex items-center gap-3 my-3 text-xl text-[var(--color-text)]">
                    {icons.map((icon, index) => (
                        <span key={index} className="h-6 w-6 hover:text-[var(--color-secondary-2)] hover:cursor-pointer">
